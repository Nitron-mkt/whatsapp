// campanhas-disparar (v44) — GIRO/resgate: bullet do rep mostra 'ticket medio R$X · Yd sem comprar' (ticket medio = faturamento_12m / num_compras, de contato_enriquecido = valor medio por pedido/reposicao). IA explica que e o quanto o cliente costuma por em cada pedido, NAO cobranca/saldo. Consolida por matriz. Exclui INAD+INTRA. MOTOR nitron=true. GARANTE [LISTA].
...
function garantirLista(p: any) { if (!/\[LISTA\]/i.test(String(p.whatsapp || ""))) p.whatsapp = String(p.whatsapp || "").replace(/\s+$/, "") + "\n\n[LISTA]"; if (!/\[LISTA\]/i.test(String(p.email_corpo || ""))) p.email_corpo = String(p.email_corpo || "").replace(/\s+$/, "") + "\n\n[LISTA]"; return p; }
...
async function modeloRep(promptIa: string, ctx: any) {
  const fb = { whatsapp: "[REP], seguem clientes seus para acionar:\n\n[LISTA]\n\nConte com a gente.", email_assunto: "[REP], seus clientes para acionar", email_corpo: "[REP],\n\nSeguem clientes seus para acionar:\n\n[LISTA]\n\nQualquer duvida, e so chamar." };
  const sys = `${BASE}\n\nTAREFA: escreva um MODELO AO REPRESENTANTE, pedindo que ELE contate/visite os clientes DELE. OBRIGATORIO: inclua o token [LISTA] no WhatsApp E no e-mail - e onde entra a lista real dos clientes dele (nome, condicao, prazo); SEM [LISTA] a mensagem NAO serve. Use tambem [REP]. Fale COM o rep, com DIRECIONAMENTO do que ele deve pedir/mobilizar. OBJETIVO: ${ctx.rep} ANGULO: ${pick(ANG_REP)} Responda APENAS JSON {whatsapp, email_assunto, email_corpo}. Curto, pt-BR.`;
